...
use flowpact_protocol::{Capabilities, Command, Parser as RiftParser, Response, FLOWPACT_VERSION};

mod chunk_tracker;

#[derive(Parser)]
#[command(name = "rift")]
#[command(about = "FLOWPACT Protocol CLI - Reliable Interoperable File Transfer")]
struct Cli {
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Raw { ref command } => {
            let mut conn = connect(&cli).await?;
            let raw_cmd = format!("FLOWPACT {}", command.join(" "));
...
async fn run_interactive_session(cli: &Cli, partner_id: &str) -> Result<()> {
    ...
    // Handshake: HELLO
    println!(">>> FLOWPACT HELLO {} CHUNKED,PARALLEL,RESUME", FLOWPACT_VERSION);
    let hello = Command::hello(FLOWPACT_VERSION, Capabilities::all());
    ...
    // Authenticate
    println!(">>> FLOWPACT AUTH {}", partner_id);
    ...
    loop {
        ...
        let cmd_line = if trimmed.starts_with("FLOWPACT ") {
            trimmed.to_string()
        } else {
            format!("FLOWPACT {}", trimmed)
        };
