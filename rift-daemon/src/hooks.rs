...
impl HookContext {
    ...
    pub fn to_env_vars(&self) -> Vec<(String, String)> {
        let mut vars = vec![
            ("RIFT_EVENT".to_string(), self.event.clone()),
            ("RIFT_TIMESTAMP".to_string(), self.timestamp.clone()),
        ];

        if let Some(ref v) = self.transfer_id {
            vars.push(("RIFT_TRANSFER_ID".to_string(), v.clone()));
        }
        if let Some(ref v) = self.partner_id {
            vars.push(("RIFT_PARTNER_ID".to_string(), v.clone()));
        }
        if let Some(ref v) = self.virtual_file {
            vars.push(("RIFT_VIRTUAL_FILE".to_string(), v.clone()));
        }
        if let Some(ref v) = self.local_path {
            vars.push(("RIFT_LOCAL_PATH".to_string(), v.clone()));
        }
        if let Some(v) = self.file_size {
            vars.push(("RIFT_FILE_SIZE".to_string(), v.to_string()));
        }
        if let Some(v) = self.chunks {
            vars.push(("RIFT_CHUNKS".to_string(), v.to_string()));
        }
        if let Some(ref v) = self.error {
            vars.push(("RIFT_ERROR".to_string(), v.clone()));
        }
        if let Some(ref v) = self.remote_addr {
            vars.push(("RIFT_REMOTE_ADDR".to_string(), v.clone()));
        }
...
impl HookManager {
    ...
    async fn run_hook(config: &HookConfig, ctx: &HookContext) -> HookResult {
        ...
        cmd.env("RIFT_CONTEXT_JSON", ctx.to_json());
...
mod tests {
    ...
    fn test_hook_context() {
        ...
        assert!(vars.iter().any(|(k, _)| k == "RIFT_EVENT"));
        assert!(vars.iter().any(|(k, _)| k == "RIFT_PARTNER_ID"));
    }
