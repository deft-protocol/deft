...
use flowpact_common::{sha256_hex, Chunker};
use flowpact_protocol::{
    AckStatus, Capabilities, Command, Parser, Response, RiftErrorCode, FLOWPACT_VERSION,
};
    ...
    fn receive_chunk(&mut self, transfer_id: &str, chunk_index: u64, data: Vec<u8>, expected_hash: &str) -> Response {
        ...
        if computed_hash != expected_hash {
            return Response::ChunkAck {
                ...
                chunk_index,
                status: AckStatus::Error(flowpact_protocol::AckErrorReason::HashMismatch),
            };
        }
...
fn test_full_handshake_and_auth() {
    ...
    // HELLO
    let hello = Command::hello(FLOWPACT_VERSION, Capabilities::all());
    let resp = session.handle_command(&hello);
    
    if let Response::Welcome { version, session_id, .. } = resp {
        assert_eq!(version, FLOWPACT_VERSION);
...
fn test_small_file_transfer() {
    ...
    // Handshake
    session.handle_command(&Command::hello(FLOWPACT_VERSION, Capabilities::all()));
...
fn test_hash_mismatch_rejection() {
    ...
    if let Response::ChunkAck { status, .. } = resp {
        assert!(matches!(status, AckStatus::Error(flowpact_protocol::AckErrorReason::HashMismatch)));
    } else {
...
fn test_protocol_parsing_roundtrip() {
    ...
    let commands = vec![
        Command::hello(FLOWPACT_VERSION, Capabilities::all().with_window_size(128)),
