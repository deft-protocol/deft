...
impl HookContext {
    ...
    pub fn to_env_vars(&self) -> Vec<(String, String)> {
        let mut vars = vec![
            ("FLOWPACT_EVENT".to_string(), self.event.clone()),
            ("FLOWPACT_TIMESTAMP".to_string(), self.timestamp.clone()),
        ];

        if let Some(ref v) = self.transfer_id {
            vars.push(("FLOWPACT_TRANSFER_ID".to_string(), v.clone()));
        }
        if let Some(ref v) = self.partner_id {
            vars.push(("FLOWPACT_PARTNER_ID".to_string(), v.clone()));
        }
        if let Some(ref v) = self.virtual_file {
            vars.push(("FLOWPACT_VIRTUAL_FILE".to_string(), v.clone()));
        }
        if let Some(ref v) = self.local_path {
            vars.push(("FLOWPACT_LOCAL_PATH".to_string(), v.clone()));
        }
        if let Some(v) = self.file_size {
            vars.push(("FLOWPACT_FILE_SIZE".to_string(), v.to_string()));
        }
        if let Some(v) = self.chunks {
            vars.push(("FLOWPACT_CHUNKS".to_string(), v.to_string()));
        }
        if let Some(ref v) = self.error {
            vars.push(("FLOWPACT_ERROR".to_string(), v.clone()));
        }
        if let Some(ref v) = self.remote_addr {
            vars.push(("FLOWPACT_REMOTE_ADDR".to_string(), v.clone()));
        }
...
impl HookManager {
    ...
    async fn run_hook(config: &HookConfig, ctx: &HookContext) -> HookResult {
        ...
        cmd.env("FLOWPACT_CONTEXT_JSON", ctx.to_json());
...
mod tests {
    ...
    fn test_hook_context() {
        ...
        assert!(vars.iter().any(|(k, _)| k == "FLOWPACT_EVENT"));
        assert!(vars.iter().any(|(k, _)| k == "FLOWPACT_PARTNER_ID"));
    }
