...
#[derive(Parser)]
#[command(name = "flowpactd")]
#[command(about = "FLOWPACT Protocol Daemon - Reliable Interoperable File Transfer")]
struct Cli {
...
async fn run_list(config: &Config, partner_id: &str) -> Result<()> {
    ...
    if let flowpact_protocol::Response::Files { files } = response {
        for file in files {
