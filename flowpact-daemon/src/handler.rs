...
impl CommandHandler {
    ...
    fn handle_hello(
        ...
        session.set_welcomed(FLOWPACT_VERSION.to_string(), negotiated.clone());

        Response::welcome(FLOWPACT_VERSION, negotiated, &session.id)
    }
    ...
    fn handle_get(
        ...
        session: &Session,
        virtual_file: String,
        chunks: flowpact_protocol::ChunkRange,
    ) -> Response {
        if !session.is_authenticated() {
            return Response::error(
                RiftErrorCode::Unauthorized,
        ...
        match transfer_id {
            Some(id) => {
                ...
                let status = self.transfer_manager.validate_chunk(&id, chunk_index, data)
                    .unwrap_or(AckStatus::Error(flowpact_protocol::AckErrorReason::Unknown));
                ...
                if status == AckStatus::Ok {
                    if let Err(e) = self.chunk_store.store_chunk(&id, chunk_index, data) {
                        ...
                        return Response::ChunkAck {
                            ...
                            chunk_index,
                            status: AckStatus::Error(flowpact_protocol::AckErrorReason::IoError),
                        };
                    }
                }
            ...
            None => {
                Response::ChunkAck {
                    virtual_file: virtual_file.to_string(),
                    chunk_index,
                    status: AckStatus::Error(flowpact_protocol::AckErrorReason::Unknown),
                }
            }
        }
    }
    ...
    pub fn complete_transfer(
        ...
        let receipt = flowpact_protocol::TransferReceipt {
