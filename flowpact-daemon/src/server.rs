...
impl Server {
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        info!("FLOWPACT server listening on {}", self.config.server.listen);
    ...
    loop {
        ...
        if let Some(partner_id) = session.partner_id() {
            if !rate_limiter.check_partner(partner_id).await.is_allowed() {
                ...
                let error_response = flowpact_protocol::Response::Error {
                    code: flowpact_protocol::RiftErrorCode::RateLimited,
        ...
        if let flowpact_protocol::Response::ChunkReady { ref virtual_file, chunk_index, size } = response {
        ...
        if matches!(response, flowpact_protocol::Response::Goodbye) {
    ...
    R: tokio::io::AsyncRead + Unpin,
{
    ...
    match reader.read_exact(&mut buffer).await {
        ...
        Err(e) => {
            ...
            flowpact_protocol::Response::ChunkAck {
                virtual_file: virtual_file.to_string(),
                chunk_index,
                status: flowpact_protocol::AckStatus::Error(flowpact_protocol::AckErrorReason::IoError),
            }
        }
    }
}
