...
use rift_protocol::{Capabilities, Command, Parser as RiftParser, Response, RIFT_VERSION};

mod chunk_tracker;

#[derive(Parser)]
#[command(name = "rift")]
#[command(about = "RIFT Protocol CLI - Reliable Interoperable File Transfer")]
struct Cli {
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Raw { ref command } => {
            let mut conn = connect(&cli).await?;
            let raw_cmd = format!("RIFT {}", command.join(" "));
...
async fn run_interactive_session(cli: &Cli, partner_id: &str) -> Result<()> {
    ...
    // Handshake: HELLO
    println!(">>> RIFT HELLO {} CHUNKED,PARALLEL,RESUME", RIFT_VERSION);
    let hello = Command::hello(RIFT_VERSION, Capabilities::all());
    ...
    loop {
        ...
        let cmd_line = if trimmed.starts_with("RIFT ") {
            trimmed.to_string()
        } else {
            format!("RIFT {}", trimmed)
        };
