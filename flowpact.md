# FlowPact Protocol - Spécification Fonctionnelle v0.1
...
FlowPact est un protocole moderne de transfert de fichiers conçu pour les échanges B2B (Business-to-Business). Il combine la simplicité architecturale de FTP avec des concepts avancés inspirés de PESIT (fichiers virtuels, partenaires) et BitTorrent (chunks, vérification d'intégrité).
...
FlowPact utilise une architecture peer où chaque instance peut agir simultanément comme client et serveur. Cette approche unifiée reflète la réalité des échanges B2B bidirectionnels.
...
Connexion TCP persistante sur le port **7741** (FlowPact en leet speak) :
...
FlowPact utilise un système d'accusés de réception asynchrones pour garantir la fiabilité sans sacrifier la performance.
...
FlowPact fournit une traçabilité complète et des preuves de transfert pour l'audit et la conformité :
...
| Fonctionnalité | FTP/SFTP | PESIT | AS2 | FlowPact |
|----------------|----------|-------|-----|------|
...
FlowPact s'inspire de plusieurs protocoles éprouvés :
...
**Composants** :
- `flowpact-protocol` : définitions du protocole
- `flowpact-daemon` : daemon unifié (serveur + client)
- `flowpact-cli` : interface en ligne de commande pour contrôler le daemon
- `flowpact-common` : utilitaires partagés (chunking, hashing)
...
### 10.1 Daemon FlowPact

Chaque instance FlowPact est un daemon unique capable d'agir comme client et serveur simultanément :
...
Dans l'architecture peer, chaque instance FlowPact maintient :
...
Pour optimiser la performance, FlowPact utilise un modèle asynchrone inspiré du contrôle de flux TCP :
...
```
FlowPact CHUNK_ACK <virtual-file> <chunk-index> <status> [<reason>]

Exemples :
FlowPact CHUNK_ACK invoices-jan 42 OK
FlowPact CHUNK_ACK invoices-jan 43 ERROR HASH_MISMATCH
FlowPact CHUNK_ACK invoices-jan 44 ERROR TIMEOUT
```

**ACK groupé (optimisation) :**
```
FlowPact CHUNK_ACK_BATCH <virtual-file> <chunk-ranges>

Exemple :
FlowPact CHUNK_ACK_BATCH invoices-jan 1-50,52-100,105-200
...
```
FlowPact TRANSFER_COMPLETE <virtual-file> <file-hash> <total-size> <chunk-count> [<signature>]

Exemple :
FlowPact TRANSFER_COMPLETE invoices-jan sha256:abc123def456... 104857600 400 sig:xyz789...
```
...
```
Client → Server:
FlowPact HELLO 1.0 CHUNKED,PARALLEL,RESUME WINDOW_SIZE:128

Server → Client:
FlowPact WELCOME 1.0 CHUNKED,PARALLEL,RESUME WINDOW_SIZE:64 sess_xyz
```
...
```
FlowPact CHUNK_ACK invoices-jan 42 ERROR HASH_MISMATCH
```
...
| FlowPact | Oui | Oui | Oui | Oui (signé) |

FlowPact combine le meilleur des mondes : fiabilité granulaire de PESIT avec performance moderne de TCP et preuves cryptographiques d'AS2.
