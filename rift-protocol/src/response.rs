        ...
        match self {
            Response::Welcome { version, capabilities, session_id } => {
                if capabilities.caps.is_empty() && capabilities.window_size.is_none() {
                    write!(f, "RIFT WELCOME {} {}", version, session_id)
                } else {
                    write!(f, "RIFT WELCOME {} {} {}", version, capabilities, session_id)
                }
            }
            Response::AuthOk { partner_name, virtual_files } => {
                let vf_list = virtual_files.join(",");
                write!(f, "RIFT AUTH_OK \"{}\" VF:{}", partner_name, vf_list)
            }
            Response::Files { files } => {
                writeln!(f, "RIFT FILES {}", files.len())?;
                for file in files {
            ...
            Response::FileInfo { info, chunks } => {
                writeln!(f, "RIFT FILE_INFO {} SIZE:{} CHUNKS:{} CHUNK_SIZE:{} HASH:{}",
            ...
            Response::ChunkData { virtual_file, chunk_index, data } => {
                write!(f, "RIFT CHUNK_DATA {} {} SIZE:{}", virtual_file, chunk_index, data.len())
            }
            Response::ChunkOk { virtual_file, chunk_index } => {
                write!(f, "RIFT CHUNK_OK {} {}", virtual_file, chunk_index)
            }
            Response::TransferAccepted { transfer_id, virtual_file, window_size } => {
                write!(f, "RIFT TRANSFER_ACCEPTED {} {} WINDOW_SIZE:{}", transfer_id, virtual_file, window_size)
            }
            Response::ChunkReady { virtual_file, chunk_index, size } => {
                write!(f, "RIFT CHUNK_READY {} {} SIZE:{}", virtual_file, chunk_index, size)
            }
            Response::ChunkAck { virtual_file, chunk_index, status } => {
                write!(f, "RIFT CHUNK_ACK {} {} {}", virtual_file, chunk_index, status)
            }
            Response::ChunkAckBatch { virtual_file, ranges } => {
                let ranges_str: Vec<String> = ranges.iter().map(|r| r.to_string()).collect();
                write!(f, "RIFT CHUNK_ACK_BATCH {} {}", virtual_file, ranges_str.join(","))
            }
            Response::TransferComplete { virtual_file, file_hash, total_size, chunk_count, signature } => {
                if let Some(sig) = signature {
                    write!(f, "RIFT TRANSFER_COMPLETE {} {} {} {} sig:{}", virtual_file, file_hash, total_size, chunk_count, sig)
                } else {
                    write!(f, "RIFT TRANSFER_COMPLETE {} {} {} {}", virtual_file, file_hash, total_size, chunk_count)
                }
            }
            Response::TransferStatus { transfer_id, virtual_file, total_chunks, received_chunks, pending_chunks } => {
                let pending_str = pending_chunks.iter()
                    ...
                    .collect::<Vec<_>>()
                    .join(",");
                write!(f, "RIFT TRANSFER_STATUS {} {} {}/{} PENDING:{}", 
                    transfer_id, virtual_file, received_chunks, total_chunks, pending_str)
            }
            Response::Error { code, message } => {
                match message {
                    Some(msg) => write!(f, "RIFT ERROR {} {}", code.code(), msg),
                    None => write!(f, "RIFT ERROR {} {}", code.code(), code.message()),
                }
            }
            Response::Goodbye => {
                write!(f, "RIFT GOODBYE")
            }
        }
    }
}
...
mod tests {
    ...
    fn test_chunk_ack_response_display() {
        ...
        assert_eq!(resp.to_string(), "RIFT CHUNK_ACK invoices 42 OK");
    }
    ...
    fn test_chunk_ack_error_response_display() {
        ...
        assert_eq!(resp.to_string(), "RIFT CHUNK_ACK invoices 5 ERROR HASH_MISMATCH");
    }
    ...
    fn test_chunk_ack_batch_response_display() {
        ...
        assert_eq!(resp.to_string(), "RIFT CHUNK_ACK_BATCH data 1-50,52-100");
    }
    ...
    fn test_transfer_complete_response_display() {
        ...
        assert_eq!(resp.to_string(), "RIFT TRANSFER_COMPLETE invoices sha256:abc123 1048576 100");
    }
    ...
    fn test_transfer_complete_with_signature_display() {
        ...
        assert_eq!(resp.to_string(), "RIFT TRANSFER_COMPLETE invoices sha256:abc123 1048576 100 sig:ed25519:xyz789");
    }
    ...
    fn test_transfer_accepted_display() {
        ...
        assert_eq!(
            resp.to_string(),
            "RIFT TRANSFER_ACCEPTED tr-abc123 invoices-jan WINDOW_SIZE:64"
        );
    }
