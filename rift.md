# RIFT Protocol - Spécification Fonctionnelle v0.1
...
RIFT est un protocole moderne de transfert de fichiers conçu pour les échanges B2B (Business-to-Business). Il combine la simplicité architecturale de FTP avec des concepts avancés inspirés de PESIT (fichiers virtuels, partenaires) et BitTorrent (chunks, vérification d'intégrité).
...
RIFT utilise une architecture peer où chaque instance peut agir simultanément comme client et serveur. Cette approche unifiée reflète la réalité des échanges B2B bidirectionnels.
...
Connexion TCP persistante sur le port **7741** (RIFT en leet speak) :
...
RIFT utilise un système d'accusés de réception asynchrones pour garantir la fiabilité sans sacrifier la performance.
...
RIFT fournit une traçabilité complète et des preuves de transfert pour l'audit et la conformité :
...
| Fonctionnalité | FTP/SFTP | PESIT | AS2 | RIFT |
|----------------|----------|-------|-----|------|
...
RIFT s'inspire de plusieurs protocoles éprouvés :
...
**Composants** :
- `rift-protocol` : définitions du protocole
- `rift-daemon` : daemon unifié (serveur + client)
- `rift-cli` : interface en ligne de commande pour contrôler le daemon
- `rift-common` : utilitaires partagés (chunking, hashing)
...
Chaque instance RIFT est un daemon unique capable d'agir comme client et serveur simultanément :
...
Dans l'architecture peer, chaque instance RIFT maintient :
...
Pour optimiser la performance, RIFT utilise un modèle asynchrone inspiré du contrôle de flux TCP :
...
```
RIFT CHUNK_ACK <virtual-file> <chunk-index> <status> [<reason>]

Exemples :
RIFT CHUNK_ACK invoices-jan 42 OK
RIFT CHUNK_ACK invoices-jan 43 ERROR HASH_MISMATCH
RIFT CHUNK_ACK invoices-jan 44 ERROR TIMEOUT
```

**ACK groupé (optimisation) :**
```
RIFT CHUNK_ACK_BATCH <virtual-file> <chunk-ranges>

Exemple :
RIFT CHUNK_ACK_BATCH invoices-jan 1-50,52-100,105-200
...
```
RIFT TRANSFER_COMPLETE <virtual-file> <file-hash> <total-size> <chunk-count> [<signature>]

Exemple :
RIFT TRANSFER_COMPLETE invoices-jan sha256:abc123def456... 104857600 400 sig:xyz789...
```
...
```
Client → Server:
RIFT HELLO 1.0 CHUNKED,PARALLEL,RESUME WINDOW_SIZE:128

Server → Client:
RIFT WELCOME 1.0 CHUNKED,PARALLEL,RESUME WINDOW_SIZE:64 sess_xyz
```
...
```
RIFT CHUNK_ACK invoices-jan 42 ERROR HASH_MISMATCH
```
...
| RIFT | Oui | Oui | Oui | Oui (signé) |

RIFT combine le meilleur des mondes : fiabilité granulaire de PESIT avec performance moderne de TCP et preuves cryptographiques d'AS2.
