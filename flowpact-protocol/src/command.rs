        ...
        match self {
            Command::Hello { version, capabilities } => {
                if capabilities.caps.is_empty() && capabilities.window_size.is_none() {
                    write!(f, "FLOWPACT HELLO {}", version)
                } else {
                    write!(f, "FLOWPACT HELLO {} {}", version, capabilities)
                }
            }
            Command::Auth { partner_id } => {
                write!(f, "FLOWPACT AUTH {}", partner_id)
            }
            Command::Discover => {
                write!(f, "FLOWPACT DISCOVER")
            }
            Command::Describe { virtual_file } => {
                write!(f, "FLOWPACT DESCRIBE {}", virtual_file)
            }
            Command::Get { virtual_file, chunks } => {
                write!(f, "FLOWPACT GET {} CHUNKS {}", virtual_file, chunks)
            }
            Command::BeginTransfer { virtual_file, total_chunks, total_bytes, file_hash } => {
                write!(f, "FLOWPACT BEGIN_TRANSFER {} {} {} {}", virtual_file, total_chunks, total_bytes, file_hash)
            }
            Command::ResumeTransfer { virtual_file, transfer_id } => {
                write!(f, "FLOWPACT RESUME_TRANSFER {} {}", virtual_file, transfer_id)
            }
            Command::GetStatus { virtual_file } => {
                write!(f, "FLOWPACT GET_STATUS {}", virtual_file)
            }
            Command::Put { virtual_file, chunk_index, size, hash, nonce, compressed } => {
                write!(f, "FLOWPACT PUT {} CHUNK {} SIZE:{} HASH:{}", virtual_file, chunk_index, size, hash)?;
                if let Some(n) = nonce {
                    write!(f, " NONCE:{}", n)?;
                }
                if *compressed {
