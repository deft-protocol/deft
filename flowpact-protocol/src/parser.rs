...
mod tests {
    ...
    #[test]
    fn test_parse_hello() {
        let cmd = Parser::parse_command("FLOWPACT HELLO 1.0 CHUNKED,PARALLEL,RESUME").unwrap();
        match cmd {
    ...
    #[test]
    fn test_parse_auth() {
        let cmd = Parser::parse_command("FLOWPACT AUTH partner-acme-corp").unwrap();
        match cmd {
    ...
    #[test]
    fn test_parse_get() {
        let cmd = Parser::parse_command("FLOWPACT GET monthly-invoices CHUNKS 0-10").unwrap();
        match cmd {
    ...
    #[test]
    fn test_parse_put() {
        let cmd = Parser::parse_command("FLOWPACT PUT invoices CHUNK 5 SIZE:1024 HASH:abc123def456").unwrap();
        match cmd {
    ...
    fn test_parse_put_compressed() {
        let cmd = Parser::parse_command("FLOWPACT PUT invoices CHUNK 5 SIZE:1024 HASH:abc123 NONCE:12345 COMPRESSED").unwrap();
        match cmd {
            Command::Put { virtual_file, chunk_index, size, hash, nonce, compressed } => {
                assert_eq!(virtual_file, "invoices");
                assert_eq!(chunk_index, 5);
    ...
    fn test_parse_welcome() {
        let resp = Parser::parse_response("FLOWPACT WELCOME 1.0 CHUNKED,PARALLEL,RESUME sess_20260119_001").unwrap();
        match resp {
    ...
    fn test_parse_auth_ok() {
        let resp = Parser::parse_response("FLOWPACT AUTH_OK \"ACME Corporation\" VF:monthly-invoices,product-catalog").unwrap();
        match resp {
    ...
    #[test]
    fn test_parse_error() {
        let resp = Parser::parse_response("FLOWPACT ERROR 401 Unauthorized - Invalid partner").unwrap();
        match resp {
    ...
    fn test_parse_hello_with_window_size() {
        let cmd = Parser::parse_command("FLOWPACT HELLO 1.0 CHUNKED,PARALLEL WINDOW_SIZE:128").unwrap();
        match cmd {
            Command::Hello { version, capabilities } => {
                assert_eq!(version, "1.0");
                assert!(capabilities.has(crate::Capability::Chunked));
    ...
    fn test_parse_welcome_with_window_size() {
        let resp = Parser::parse_response("FLOWPACT WELCOME 1.0 CHUNKED,RESUME WINDOW_SIZE:64 sess_123").unwrap();
        match resp {
            Response::Welcome { version, capabilities, session_id } => {
                assert_eq!(version, "1.0");
                assert!(capabilities.has(crate::Capability::Chunked));
    ...
    fn test_parse_chunk_ack_ok() {
        let resp = Parser::parse_response("FLOWPACT CHUNK_ACK invoices 42 OK").unwrap();
        match resp {
    ...
    fn test_parse_chunk_ack_error() {
        let resp = Parser::parse_response("FLOWPACT CHUNK_ACK invoices 5 ERROR HASH_MISMATCH").unwrap();
        match resp {
            Response::ChunkAck { virtual_file, chunk_index, status } => {
                assert_eq!(virtual_file, "invoices");
                assert_eq!(chunk_index, 5);
    ...
    fn test_parse_chunk_ack_batch() {
        let resp = Parser::parse_response("FLOWPACT CHUNK_ACK_BATCH monthly-data 1-50,52-100").unwrap();
        match resp {
    ...
    fn test_parse_transfer_complete() {
        let resp = Parser::parse_response(
            "FLOWPACT TRANSFER_COMPLETE invoices sha256:abc123 1048576 100"
        ).unwrap();
        match resp {
    ...
    fn test_parse_transfer_complete_with_signature() {
        let resp = Parser::parse_response(
            "FLOWPACT TRANSFER_COMPLETE invoices sha256:abc123 1048576 100 sig:ed25519:xyz789"
        ).unwrap();
        match resp {
            Response::TransferComplete { virtual_file, file_hash, total_size, chunk_count, signature } => {
                assert_eq!(virtual_file, "invoices");
    ...
    fn test_parse_begin_transfer() {
        let cmd = Parser::parse_command(
            "FLOWPACT BEGIN_TRANSFER invoices-jan 100 10485760 sha256:abcdef123456"
        ).unwrap();
        match cmd {
    ...
    fn test_begin_transfer_display() {
        ...
        assert_eq!(
            cmd.to_string(),
            "FLOWPACT BEGIN_TRANSFER data-file 50 5242880 sha256:xyz789"
        );
    }
}
