...
Complete guide to deploying RIFT in production.
...
|------|----------|-------------|
| 7741 | TCP/TLS | RIFT protocol (configurable) |
...
sudo chmod +x /usr/local/bin/riftd
...
RIFT requires mTLS (mutual TLS) for all connections. You need:

1. **CA certificate** - Signs all partner certificates
2. **Server certificate** - Identifies your RIFT instance
...
openssl req -new -x509 -days 3650 -key ca.key -out ca.crt \
    -subj "/C=FR/O=YourCompany/CN=RIFT Internal CA"
...
```bash
riftd --config /etc/rift/config.toml
```

### With Custom Log Level

```bash
riftd --config /etc/rift/config.toml --log-level debug
```
...
```ini
[Unit]
Description=RIFT Protocol Daemon
...
User=rift
Group=rift
ExecStart=/usr/local/bin/riftd --config /etc/rift/config.toml
Restart=always
RestartSec=5
...
riftd --config /path/to/partner-config.toml \
...
# Check process
if ! pgrep -x riftd > /dev/null; then
    echo "CRITICAL: riftd not running"
    exit 2
fi
...
- Open an issue: https://github.com/yourorg/rift/issues
